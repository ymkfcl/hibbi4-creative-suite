...
const mangaGenerator_1 = require("../utils/mangaGenerator");
...
function MangaBuilder() {
    ...
    const [mangaBook, setMangaBook] = (0, react_1.useState)({
        id: 'manga-' + Date.now(),
        title: 'Mon Manga',
        author: 'Auteur',
        genre: 'Aventure',
    ...
    const [selectedPanelId, setSelectedPanelId] = (0, react_1.useState)(null);
    const [isGenerating, setIsGenerating] = (0, react_1.useState)(false);
    const [showCharacterEditor, setShowCharacterEditor] = (0, react_1.useState)(false);
    const [showStyleEditor, setShowStyleEditor] = (0, react_1.useState)(false);
    const [showTemplates, setShowTemplates] = (0, react_1.useState)(false);
    // Références
    const imageGenerator = (0, react_1.useRef)(new mangaGenerator_1.MangaImageGenerator());
    const characterConsistency = (0, react_1.useRef)(new mangaGenerator_1.CharacterConsistency());
    const pdfExporter = (0, react_1.useRef)(new pdfExporter_1.MangaPDFExporter());
    ...
    const addPage = (0, react_1.useCallback)(() => {
        const newPage = {
            id: 'page-' + Date.now(),
    ...
    const deletePage = (0, react_1.useCallback)((pageIndex) => {
    ...
    const addPanel = (0, react_1.useCallback)(() => {
        const newPanel = {
            id: 'panel-' + Date.now(),
            prompt: '',
            dialogues: [],
    ...
    const updatePanel = (0, react_1.useCallback)((panelId, updates) => {
        setMangaBook(prev => {
            const newPages = [...prev.pages];
    ...
    const deletePanel = (0, react_1.useCallback)((panelId) => {
        setMangaBook(prev => {
            const newPages = [...prev.pages];
    ...
    // Génération d'images
    const generatePanelImage = (0, react_1.useCallback)(async (panelId) => {
        const panel = mangaBook.pages[currentPageIndex].panels.find(p => p.id === panelId);
        if (!panel || !panel.prompt.trim())
            return;
        setIsGenerating(true);
        try {
            const enhancedPrompt = mangaGenerator_1.MangaPromptGenerator.generateContextualPrompt(panel.prompt, mangaBook.characters, panel.mood, mangaBook.style, panel.cameraAngle);
    ...
    const generateAllPanelImages = (0, react_1.useCallback)(async () => {
    ...
    // Ajout de dialogue
    const addDialogue = (0, react_1.useCallback)((panelId) => {
        const newDialogue = {
    ...
    const applyTemplate = (0, react_1.useCallback)((template) => {
        const newPanels = template.panels.map((templatePanel, index) => ({
    ...
    // Export PDF
    const exportToPDF = (0, react_1.useCallback)(async () => {
        try {
    ...
    return (<div className="manga-builder">
      <div className="manga-builder__header">
        ...
        <div className="manga-builder__toolbar">
          <input type="text" value={mangaBook.title} onChange={(e) => setMangaBook(prev => ({ ...prev, title: e.target.value }))} className="manga-builder__title-input" placeholder="Titre du manga"/>
      ...
      <div className="manga-builder__content">
        ...
        <div className="manga-builder__sidebar-left">
          ...
          <div className="page-list">
            {mangaBook.pages.map((page, index) => (<div key={page.id} className={`page-item ${index === currentPageIndex ? 'active' : ''}`} onClick={() => setCurrentPageIndex(index)}>
                <div className="page-thumbnail">
                  <div className="page-preview">
                    {page.panels.map(panel => (<div key={panel.id} className="panel-mini" style={{
                ...
                e.stopPropagation();
                deletePage(index);
            }} className="btn-delete" disabled={mangaBook.pages.length <= 1}>
                  ×
                </button>
              </div>))}
          </div>
          ...
          <div className="page-editor">
            <div className="page-editor__header">
              ...
              <div className="page-controls">
                ...
                <button onClick={generateAllPanelImages} className="btn btn-primary" disabled={isGenerating}>
            ...
            <div className="page-canvas">
              <div className="page-container">
                {currentPage.panels.map(panel => (<div key={panel.id} className={`panel ${selectedPanelId === panel.id ? 'selected' : ''}`} style={{
                left: `${panel.position.x}%`,
                top: `${panel.position.y}%`,
                width: `${panel.size.width}%`,
                height: `${panel.size.height}%`,
                ...
                backgroundColor: panel.backgroundColor || '#ffffff'
                    ...
                    {panel.generatedImage ? (<img src={panel.generatedImage} alt="Panel généré" className="panel-image"/>) : (<div className="panel-placeholder">
                        <p>{panel.prompt || 'Cliquez pour éditer'}</p>
                      </div>)}

                    {/* Bulles de dialogue */}
                    {panel.dialogues.map(dialogue => (<div key={dialogue.id} className={`dialogue-bubble dialogue-${dialogue.type}`} style={{
                    ...
                    {selectedPanelId === panel.id && (<div className="panel-controls">
                        <button onClick={() => generatePanelImage(panel.id)} className="btn btn-sm btn-primary" disabled={isGenerating || !panel.prompt.trim()}>
                          🎨
                        </button>
                        <button onClick={() => addDialogue(panel.id)} className="btn btn-sm btn-secondary">
                          💬
                        </button>
                        <button onClick={() => deletePanel(panel.id)} className="btn btn-sm btn-danger">
                          🗑️
                        </button>
                      </div>)}
                  </div>))}
        ...
        <div className="manga-builder__sidebar-right">
          {selectedPanel ? (<div className="panel-properties">
              <h3>Propriétés du Panel</h3>
              
              <div className="property-group">
                <label>Prompt de génération</label>
                <textarea value={selectedPanel.prompt} onChange={(e) => updatePanel(selectedPanel.id, { prompt: e.target.value })} placeholder="Décrivez la scène à générer..." rows={4}/>
              </div>

              <div className="property-group">
                <label>Angle de caméra</label>
                <select value={selectedPanel.cameraAngle} onChange={(e) => updatePanel(selectedPanel.id, { cameraAngle: e.target.value })}>
              ...
              <div className="property-group">
                ...
                <select value={selectedPanel.mood} onChange={(e) => updatePanel(selectedPanel.id, { mood: e.target.value })}>
              ...
              <div className="property-group">
                ...
                <select value={selectedPanel.borderStyle} onChange={(e) => updatePanel(selectedPanel.id, { borderStyle: e.target.value })}>
                  <option value="normal">Normale</option>
              ...
              <div className="property-group">
                ...
                <div className="position-controls">
                  <div className="input-row">
                    <label>X:</label>
                    <input type="number" value={selectedPanel.position.x} onChange={(e) => updatePanel(selectedPanel.id, {
                position: { ...selectedPanel.position, x: parseInt(e.target.value) || 0 }
            })} min="0" max="100"/>
                    <label>Y:</label>
                    <input type="number" value={selectedPanel.position.y} onChange={(e) => updatePanel(selectedPanel.id, {
                position: { ...selectedPanel.position, y: parseInt(e.target.value) || 0 }
            })} min="0" max="100"/>
                  </div>
                  <div className="input-row">
                    <label>L:</label>
                    <input type="number" value={selectedPanel.size.width} onChange={(e) => updatePanel(selectedPanel.id, {
                size: { ...selectedPanel.size, width: parseInt(e.target.value) || 10 }
            })} min="10" max="100"/>
                    <label>H:</label>
                    <input type="number" value={selectedPanel.size.height} onChange={(e) => updatePanel(selectedPanel.id, {
              ...
              {/* Dialogues */}
              {selectedPanel.dialogues.length > 0 && (<div className="property-group">
                  <label>Dialogues</label>
                  {selectedPanel.dialogues.map((dialogue, index) => (<div key={dialogue.id} className="dialogue-editor">
                      <textarea value={dialogue.text} onChange={(e) => {
                      ...
                      <select value={dialogue.type} onChange={(e) => {
                        const newDialogues = [...selectedPanel.dialogues];
                        newDialogues[index].type = e.target.value;
                        updatePanel(selectedPanel.id, { dialogues: newDialogues });
                    }}>
      ...
      {/* Modales */}
      {showTemplates && (<div className="modal-overlay" onClick={() => setShowTemplates(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-content">
              <div className="template-grid">
                {templates.map(template => (<div key={template.id} className="template-card" onClick={() => applyTemplate(template)}>
                    <div className="template-preview">
                      {template.panels.map((panel, index) => (<div key={index} className="template-panel" style={{
                        left: `${panel.position.x}%`,
                        top: `${panel.position.y}%`,
                        width: `${panel.size.width}%`,
                        height: `${panel.size.height}%`
