...
exports.MangaPDFExporter = void 0;
class MangaPDFExporter {
    constructor() {
    ...
    async initializePDF() {
        if (!this.jsPDF) {
            ...
            const jsPDFModule = await Promise.resolve().then(() => __importStar(require('jspdf')));
     ...
     */
    async addCoverPage(pdf, book, dimensions) {
        // Fond blanc
        ...
        if (book.coverImage) {
            try {
                ...
                const imgDimensions = this.calculateImageDimensions(imgData, dimensions.width - 20, dimensions.height - 60);
                pdf.addImage(book.coverImage, 'PNG', (dimensions.width - imgDimensions.width) / 2, 20, imgDimensions.width, imgDimensions.height);
            }
            catch (error) {
     ...
     */
    async addMangaPage(pdf, page, settings, dimensions) {
        ...
        pdf.text(page.pageNumber.toString(), dimensions.width - margins.right - 10, dimensions.height - margins.bottom + 5, { align: 'right' });
    }
    /**
     * Dessine un panel sur la page PDF
     */
    async drawPanel(pdf, panel, margins, contentWidth, contentHeight, gutterSize) {
        ...
        if (panel.generatedImage) {
            try {
                const imgDimensions = this.calculateImageDimensions(panel.generatedImage, width - 4, height - 4);
                pdf.addImage(panel.generatedImage, 'PNG', x + 2, y + 2, imgDimensions.width, imgDimensions.height);
            }
            catch (error) {
     ...
     */
    drawPanelBorder(pdf, x, y, width, height, style) {
     ...
     */
    drawJaggedBorder(pdf, x, y, width, height) {
        const jagSize = 2;
        // Haut
     ...
     */
    drawDialogueBubble(pdf, dialogue, panelX, panelY, panelWidth, panelHeight) {
     ...
     */
    drawPanelEffect(pdf, effect, panelX, panelY, panelWidth, panelHeight) {
        switch (effect.type) {
            case 'speed-lines':
     ...
     */
    drawSpeedLines(pdf, x, y, width, height, intensity) {
        pdf.setDrawColor(0, 0, 0);
     ...
     */
    drawImpactEffect(pdf, x, y, width, height, intensity) {
        pdf.setDrawColor(0, 0, 0);
        pdf.setLineWidth(1);
        const centerX = x + width / 2;
        const centerY = y + height / 2;
     ...
     */
    async addEndPage(pdf, book, dimensions) {
        pdf.addPage();
        // Fond blanc
        pdf.setFillColor(255, 255, 255);
        pdf.rect(0, 0, dimensions.width, dimensions.height, 'F');
        ...
        pdf.setFontSize(8);
        pdf.setTextColor(128, 128, 128);
        pdf.text(`Généré avec HIBBI Manga Builder - ${new Date().toLocaleDateString()}`, dimensions.width / 2, dimensions.height - 20, { align: 'center' });
    }
    /**
...
exports.MangaPDFExporter = MangaPDFExporter;
