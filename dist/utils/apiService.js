...
class APIService {
    ...
    async saveToHistory(params, imageBase64) {
        if (!params.userId)
            return;
        try {
            const imageUrl = `data:image/png;base64,${imageBase64}`;
            await supabase_1.history.addGeneration(params.userId, params.prompt, imageUrl, {
                width: params.width || 512,
                height: params.height || 512,
                steps: params.steps || 20,
                cfg_scale: params.cfg_scale || 7,
    ...
    getStyleFromPrompt(prompt) {
        if (prompt.includes('manga') || prompt.includes('anime'))
            return 'manga';
        if (prompt.includes('cyberpunk') || prompt.includes('sci-fi'))
            return 'cyberpunk';
    ...
    drawIntelligentPlaceholder(ctx, width, height, colors, style, prompt) {
        // Fond dégradé
    ...
    addStyleElements(ctx, width, height, style, colors) {
        ctx.globalAlpha = 0.3;
        switch (style) {
            case 'manga':
                // Lignes de vitesse
    ...
    addDemoWatermark(ctx, width, height) {
    ...
    async getSamplers() {
        try {
            ...
            const response = await fetch(`${this.baseUrl}/samplers`);
            const data = await response.json();
            return data.map((sampler) => sampler.name);
        }
        catch (error) {
            return ['Euler a', 'DPM++ 2M Karras', 'DDIM'];
        }
    ...
    async getModels() {
        try {
            ...
            const data = await response.json();
            return data.map((model) => model.title);
        }
        catch (error) {
            return ['stable-diffusion-v1-5', 'anything-v4.5'];
        }
