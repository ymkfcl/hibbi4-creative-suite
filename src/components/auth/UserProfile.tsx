...
import { history, GenerationHistory } from '../../lib/supabase';
...
const UserProfile: React.FC<UserProfileProps> = ({ isOpen, onClose }) => {
  ...
  const [userHistory, setUserHistory] = useState<GenerationHistory[]>([]);
  ...
  const loadHistory = async () => {
    ...
    setLoading(true);
    try {
      const historyData = await history.getUserHistory(user.id, 50);
  ...
  const handleDeleteGeneration = async (generationId: string) => {
    ...
    const success = await history.deleteGeneration(generationId, user.id);
    if (success) {
      ...
      <div className="profile-modal" onClick={(e) => e.stopPropagation()}>
        <div className="profile-header">
          <div className="profile-title">
            <Logo size="small" variant="default" showText={false} />
            <h2>Mon Profil</h2>
          </div>
